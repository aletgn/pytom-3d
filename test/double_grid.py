...
g_bot.add_points(distance3, std_noise=0.01)
# v.scatter3D([g_bot])
...
g_bot.fit(gpr(kernel=kernel, n_restarts_optimizer=100))

g_test = Topography()
g_test.make_grid(x_bounds=[-l,l], y_bounds=[h,H], x_res=x_res*2, y_res=y_res*2)
# g_bot.regressor.kernel_.get_params() # list parameters

z, sigma = g_bot.pred(g_test.P[:,0:2])
...
v.scatter3DRegression(g_eval, g_bot)
